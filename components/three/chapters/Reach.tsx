...
/** 05 — Reach: on a classical plinth at golden hour, reaching for what comes next. */
...
export default function Reach({ index, camera }: ChapterProps) {
  ...
  const rest = useMemo(() => new THREE.Vector3(-0.7, -0.2, 8.2), []);
  const from = useMemo(() => new THREE.Vector3(0.2, 3.5, 3), []);
  const look = useMemo(() => new THREE.Vector3(-0.3, 0.1, 0), []);
  ...
  useFrame(() => {
    ...
    const rise = 1 - arrive;
    stage.focus[index].set(-1.25, 1.75, 0.8);
    stage.anchor[index].set(0, 0.8, 0);

    if (orb.current) {
      orb.current.position.set(-1.25 + Math.sin(t * 0.4) * 0.06, 1.75 + Math.sin(t * 0.9) * 0.08 - rise * 1.5, 0.8);
  ...
  return (
    <>
      ...
      <group position={[0.4, -0.2, 0]} rotation={[0, -0.45, 0]}>
        <Android index={index} lookUp={0.28} />
        <Column />
      </group>
